...
*/
    ...
    const Int_t T = 30;
    const Int_t L = 6;
    const Int_t windowSize = 2 * setting.TRise; // region where time stamp locates
    ...
    // Float_t height(0);
    // std::ofstream newpulseFile("interpolatedPulse");
    // Float_t threshold(0);
    ...
    startIndex = startIndex / 100 - setting.TRise;
    ...
    for (int k = 0; k < events.size(); k++)
    {
        ...
        if (!events[k].isBad) {
            ...
            //     // Int_t startIndex = events[0].heightindex - setting.TRise;
            //     for(int j = 0; j < N; j++)
            //     {
        ...
        /* 
            ...
            temp = f * newpulse[tempIndex];
            for(int j = tempIndex;j > 0;j--)
            {
                if(newpulse[j] > temp && newpulse[j-1] <= temp)
                {
                    events[k].timeStampDACFD = ( j - (newpulse[j] - temp) / (newpulse[j] - newpulse[j-1]) ) * delT;
