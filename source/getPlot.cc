...
Int_t getPSD(TH2F *histo, TCanvas *canvas, const Parameters &setting, const std::vector<Event> &events, const float_t calicoef, std::function<Bool_t(Event)> cond)
{
    ...
    // graph->Draw("Ap");
    histo -> GetYaxis()->SetRangeUser(0., 0.6);
    // histo->SetStats(0);
    canvas->SetLogz();
    histo -> Draw("COLZ");
    canvas -> Draw();
...
Int_t plotPulse(const Parameters &setting, const std::vector<std::vector< Float_t > > &Pulses,std::string plotname)
{
    ...
    const UInt_t M = (10 < Pulses.size() ? 10 : Pulses.size());

    std::vector<std::vector<Float_t> > y_vals(M);
    TGraph *graphs[M+2];
    UInt_t heightindex(0);
    Float_t height(0);
    // UInt_t good_counts = 0;
    for(int i=0; i < M;i++)
    {
        // calculate pulse height and height index
        heightindex = std::max_element(Pulses[i].begin(), Pulses[i].end()) - Pulses[i].begin();
        height = *(Pulses[i].begin() + heightindex);
        for (int j = 0; j < N; j++)
        {
            y_vals[i].push_back( Pulses[i][j] );
        }
        graphs[i] = new TGraph(N, x_vals, &(y_vals[i][0] ));
        // graphs[good_counts]->SetMarkerStyle(20);
        // graphs[i]->SetLineColor(kRed + i* 2);
